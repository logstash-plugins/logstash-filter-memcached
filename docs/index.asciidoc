 ...
 * `get`: get values for one or more memcached keys and inject them into the event at the
 provided paths. An in-process LRU cache can optionally also be added to significantly
 reduce load and round-trip times to memcached.
...
| <<plugins-{type}s-{plugin}-lru_cache_ttl>> |<<number,number>>|No
| <<plugins-{type}s-{plugin}-lru_cache_max_entries>> |<<number,number>>|No
...
[id="plugins-{type}s-{plugin}-lru_cache_ttl"]
===== `lru_cache_ttl`

  * Value type is <<number,number>>
  * Default is 0, which disables this functionality.

If greater than 0, and if `lru_cache_max_entries` is also greater than 0, data retrieved from memcached will be stored in a least-recently-used cache that is thread-local to the process, which can radically reduce the number of calls between logstash and memcached, particularly if the majority of your lookups are for the same keys that don't change much. This option specifies the time, in seconds, that the data should remain cached locally by this thread.

Memory overhead and performance gain will determine on traffic patterns and size of the response from memcached. You should investigate monitoring performance using tools such as Logstash's REST API. There is no current mechanism to expose how many items are currently used by the cache.
...
[id="plugins-{type}s-{plugin}-lru_cache_max_entries"]
===== `lru_cache_max_entries`

  * Value type is <<number,number>>
  * Default is 0, which disables this functionality.

If greater than 0, and if `lru_cache_ttl` is also greater than 0, data retrieved from memcached will be stored in a least-recently-used cache that is thread-local to the process, which can radically reduce the number of calls between logstash and memcached, particularly if the majority of your lookups are for the same keys that don't change much. This option specifies the maximum number of entries that will be kept in the cache, particular to this thread.
